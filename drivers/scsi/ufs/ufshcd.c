		...
		u8 *buf, u32 *size)
{
	...
	dev_err(hba->dev,
			"%s is %s!! Query Function=0x%x err = %d\n",
			__func__, (err)? "Failed":"OK", query_fn, err);
		...
		case UPIU_TRANSACTION_RESPONSE:
			...
			if (!hba->pm_op_in_progress &&
				...
				if (queue_work(hba->recovery_wq,
							&hba->eeh_work)) {
					pm_runtime_get_noresume(hba->dev);
					dev_err(hba->dev, "exception event reported\n");
				}
			}
			break;
		case UPIU_TRANSACTION_REJECT_UPIU:
...
#endif
	/*
	 * Most of UFS vendors need about 2seconds 
	 * for gaurantee auto flush before full reset(host and device) 
	 */
	ssleep(2);
	ufshcd_enable_irq(hba);
